...
export default function SuperAdminDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [st, t, es] = await Promise.all([
        getPlatformStats(),
        getAllTenants(),
        getErrorStats().catch(() => ({ critical: 0, warning: 0, info: 0, total: 0 })),
      ]);
      setStats(st);
      setErrStats(es);
      ...
      <nav style={s({ borderBottom: '1px solid var(--sa-border)', background: 'var(--sa-bg-card)', backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 50 })}>
        <div style={s({ maxWidth: 1200, margin: '0 auto', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 60 })}>
          ...
          <div style={s({ display: 'flex', alignItems: 'center', gap: 6 })}>
            {/* Overview tab */}
            <button onClick={() => setView('overview')} style={s({ padding: '7px 14px', borderRadius: 8, background: view === 'overview' ? 'var(--sa-bg-elevated)' : 'transparent', border: `1px solid ${view === 'overview' ? 'var(--sa-border)' : 'transparent'}`, cursor: 'pointer', color: view === 'overview' ? 'var(--sa-text)' : 'var(--sa-text-muted)', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, transition: 'all 0.2s' })}>
              <Building2 size={13} /> Overview
            </button>

            {/* Error Logs tab */}
            <button onClick={() => setView('errors')} style={s({ position: 'relative', padding: '7px 14px', borderRadius: 8, background: view === 'errors' ? 'rgba(248,113,113,0.1)' : 'transparent', border: `1px solid ${view === 'errors' ? 'rgba(248,113,113,0.3)' : 'transparent'}`, cursor: 'pointer', color: view === 'errors' ? '#f87171' : 'var(--sa-text-muted)', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, transition: 'all 0.2s' })}>
              <AlertTriangle size={13} /> Error Logs
              {errStats.critical > 0 && (
                <span style={s({ position: 'absolute', top: -5, right: -5, minWidth: 17, height: 17, borderRadius: 9, background: '#f87171', color: 'white', fontSize: 9, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 4px', lineHeight: 1 })}>
                  {errStats.critical > 99 ? '99+' : errStats.critical}
                </span>
              )}
            </button>
