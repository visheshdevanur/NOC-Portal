    ...
    if (action?.startsWith('coe-')) {
      ...
      switch (action) {
        ...
        case 'coe-validate-csv': {
          ...
          const usnArr = [...new Set(rows.map((r: any) => r.usn?.trim().toUpperCase()).filter(Boolean))]
          const codeArr = [...new Set(rows.map((r: any) => r.subject_code?.trim().toUpperCase()).filter(Boolean))]
          ...
          if (usnArr.length > 0) {
            ...
            ;(students || []).forEach((s: any) => usnMap.set(s.roll_number?.toUpperCase(), s.id))
          }
          if (codeArr.length > 0) {
            const { data: subjects } = await coeClient.from('subjects').select('id, subject_code').in('subject_code', codeArr)
            ;(subjects || []).forEach((s: any) => codeMap.set(s.subject_code?.toUpperCase(), s.id))
          }
